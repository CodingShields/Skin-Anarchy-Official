...
const SignUp = () => {
	...
	const handleSubmit = async (e) => {
		...
		if (!noEmptyFields) {
			setState({ ...state, error: true, errorMessage: "Please fill out all fields" });
		} else if (form.password.length < 6) {
			setState({ ...state, error: true, errorMessage: "Password must be at least 6 characters" });
		} else if (form.password.length > 20) {
			setState({ ...state, error: true, errorMessage: "Password must be less than 20 characters" });
		} else if (form.password.search(/[a-z]/i) < 0) {
			setState({ ...state, error: true, errorMessage: "Password must contain at least one letter" });
		} else if (form.password.search(/[0-9]/) < 0) {
			setState({ ...state, error: true, errorMessage: "Password must contain at least one digit" });
		} else if (form.password.search(/[!@#$%^&*]/) < 0) {
			setState({ ...state, error: true, errorMessage: "Password must contain at least one special character" });
		} else if (form.password.search(/[^a-zA-Z0-9!@#$%^&*]/) > 0) {
			setState({ ...state, error: true, errorMessage: "Password must not contain spaces" });
		}
		try {
			...
			if (error.code === "auth/email-already-in-use") {
				setState({ ...state, error: true, errorMessage: "User already exists with this email" });
	...
	return (
		<div className='flex flex-col pt-60 h-fit'>
			...
			<form className='flex flex-col justify-start items-center mt-10 space-y-10 '>
				<fieldset>
					<legend className='text-sm font-semibold leading-6 text-gray-900'>Personal Information</legend>
