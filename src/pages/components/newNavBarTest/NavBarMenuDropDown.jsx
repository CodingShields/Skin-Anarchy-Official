	...
	return open ? (
		<div className={menuStyle ? menuStyle : null}>
			<div onMouseOver={handleMouseOver} className={navMenu}>
				{titleText ? <h1 className=' text-center text-3xl underline pt-8 underline-offset-8 decoration-1'>{titleText}</h1> : null}
				{children}
			</div>
			{/* <div className={highlightMenuStyle ? highlightMenuStyle : null}>
				{podcast[index === 0 ? 0 : index].map((item, index) => {
					return (
						<div key={index} className='w-fit text-center mx-auto pl-12'>
							<img src={item.highlight.image} className='w-72' />
							<h1 className='text-white text-lg font-montserrat py-8'>{item.highlight.guest}</h1>
						</div>
					);
				})}
			</div> */}
