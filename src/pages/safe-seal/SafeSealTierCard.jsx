...
const SafeSealTierCard = (props) => {
	...
	const icon = <CheckBadgeIcon className='w-10 h-10 inline-block stroke-gold-500' />;
	const buttonStyle =
	...
	return (
		<div
			className={`w-[900px] border px-4 py-2 rounded-bl-xl rounded-tr-xl rounded-br-xl bg-black relative ${open ? "h-[900px]" : "h-56"} transition-all ease-in-out duration-300 group`}
		>
			...
			<img src={safeSeal} className='absolute  w-full h-full object-cover opacity-10  transition-all ease-in-out duration-300 top-0 left-20' alt='safe seal' />
			<ul className={`list-outside space-y-4 py-4 w-[600px] list-image-${icon}`}>
				<li className='text-lg indent-5'>
					{" "}
					<CheckBadgeIcon className='w-10 h-10 inline-block stroke-gold-500' /> {option1}
				</li>
				<li className='text-lg indent-5'>
					{" "}
					<CheckBadgeIcon className='w-10 h-10 inline-block stroke-gold-500 line-clamp-6' /> {option2}
				</li>
			</ul>
