	...
	return (
		<div className='flex flex-col h-fit w-full items-center content-center justify-center grid-cols-3 space-y-20 bg-black pb-40'>
			<div className='w-full h-fit py-20'>
				<h1 className=' text-4xl font-bold text-center text-white mt-20 mb-6'>
			...
			<div className='flex flex-row content-start justify-center w-full h-full py-12 '>
				{factsBarData.map((item) => (
					<div key={item.id} className='flex flex-col items-center w-fit h-2/3 '>
						<img className='w-20 h-20 pt-0 ' src={item.icon} />
						<h3 className='mt-2 text-2xl font-bold text-center text-gray-600 hover:text-white transition-all ease-in-out duration-200'>{item.title}</h3>
