...
const FactsBar = () => {
  ...
  return (
    <div className="flex flex-col h-fit w-full items-center content-center justify-center grid-cols-3 py-10 bg-black ">
      ...
      {barData.map((item) => (
        <div
          key={item.id}
          className="flex flex-row content-start justify-center w-full h-full py-12"
        >
          <div className="flex flex-col items-center w-2/3 h-2/3">
            <img className="w-20 h-20 pt-0" src={item.icon} />
            <h3 className="mt-2 text-2xl font-bold text-center text-gray-600 hover:text-white">
              {item.title}
            </h3>
