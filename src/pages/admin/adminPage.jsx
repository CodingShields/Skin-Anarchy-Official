...
import AdminNewsLetter from "./comp/adminNewsLetter";
...
const navigation = [
	...
	{ name: "Stats", icon: ChartBarSquareIcon, helperMessage: "Check Current Podcast Stats" },
	{ name: "Blog", icon: BookOpenIcon, helperMessage: "Update Blog" },
	{ name: "News Letter", icon: NewspaperIcon, helperMessage: "Send Weekly NewsLetter" },
	{ name: "Return To Website", icon: ArrowUturnLeftIcon, helperMessage: "Return To WebSite" },
];
...
const AdminPage = () => {
	...
	return (
		<div className='flex flex-row h-full w-full'>
				...
				<div className='flex flex-col items-center justify-center  h-full w-full bg-gray-700 mt-40'>
					<div className='flex flex-col items-start justify-start  h-full w-full mt-20 px-4'>
						{navigation.map((item, index) => {
							return (
								<div
									onClick={(e) => handleNavigation(item.name)}
			...
			<div className='flex flex-col  w-full h-full bg-gray-200'>{renderStep}</div>
		</div>
	);
};
